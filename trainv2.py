...
def train(args, model):
    ...
    for epoch in range(args.num_epochs):
        history['tr_loss'] = []
        #model.module.reset_states(args.batch_size) # 每个epoch开始时重置隐藏状态
        ...
        print("Epoch: %d ,  loss: %.10f " % (epoch, np.mean(history['tr_loss'])))
...
def main(input_file):
    ...
    save_model(model, 'model/bazarbackdoor/examplev3_generator_1.pth')
    
    with open('pickle/bazarbackdoor/examplev3_char_mappings_1.pkl', 'wb') as f:
        pickle.dump((char2idx, idx2char, args), f)
        
    print(f'Model saved to model/bazarbackdoor/examplev3_generator_1.pth')

if __name__ == "__main__":
    main("data/bazarbackdoor/examplev3.txt")
